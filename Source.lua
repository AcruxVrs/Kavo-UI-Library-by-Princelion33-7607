...
-- To use toggle remembering do Kavo:Init() at the end of script (all controls must be loaded to be able to set them)
local Kavo = {
    flags = {},
 ...
 --   Kavo.SaveCfg = true
    ...
    if Kavo.SaveCfg then
        if not isfolder(Kavo.Folder) then
            makefolder(Kavo.Folder)
        end
    end

    function SaveCfg(Name)
        local Data = {}
        for flag, value in pairs(Kavo.flags) do
            task.wait()
            Data[flag] = value.Value
        end
        writefile(Kavo.Folder .. "/" .. Name .. ".txt", tostring(HttpService:JSONEncode(Data)))
    end
    ...
    function LoadCfg(Config)
        ...
        for flag, value in pairs(Data) do
            task.wait()
            if Kavo.flags[flag] then
                spawn(function()
                    Kavo.flags[flag]:UpdateToggle(nil, value)
                end)
            else
                warn("Could not find flag:", flag)
                print("Available flags:")
                for flagName, _ in pairs(Kavo.flags) do
                    task.wait()
                    print(flagName)
                end
            end
    ...
    function Kavo:Init()
        task.spawn(function()
            if Kavo.SaveCfg then
                pcall(function()
                    if isfile(Kavo.Folder .. "/" .. "DBU" .. ".txt") then
                        LoadCfg(readfile(Kavo.Folder .. "/" .. "DBU" .. ".txt"))
        ...
        coroutine.wrap(function()
            ...
            function Elements:NewTextBox(tname, tTip, callback)
                ...
                function Elements:NewToggle(tname, nTip, callback)
                    ...
                    callback = callback or function() end
                    local TogFunction, Toggleds = { Value = false }, false
                ...
                UpdateSize()
                    ...
                    btn.MouseButton1Click:Connect(function()
                        if not focusing then
                            if TogFunction.Value == false then
                            ...
                            TogFunction.Value = not TogFunction.Value
                            SaveCfg("DBU")
                            pcall(callback, TogFunction.Value)
                        else
                            for i,v in next, infoContainer:GetChildren() do
                                Utility:TweenObject(v, {Position = UDim2.new(0,0,2,0)}, 0.2)
                                focusing = false
                    ...
                    function TogFunction:UpdateToggle(newText, isTogOn)
                        isTogOn = isTogOn or TogFunction.Value
                        if newText ~= nil then 
                            togName.Text = newText
                        end
                        if isTogOn then
                            TogFunction.Value = true
                            game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear,Enum.EasingDirection.In), {
                                ImageTransparency = 0
                            }):Play()
                            pcall(callback, TogFunction.Value)
                        else
                            TogFunction.Value = false
                            game.TweenService:Create(img, TweenInfo.new(0.11, Enum.EasingStyle.Linear,Enum.EasingDirection.In), {
                                ImageTransparency = 1
                            }):Play()
                            pcall(callback, TogFunction.Value)
